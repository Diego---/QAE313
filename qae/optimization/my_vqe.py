...
from qiskit_algorithms.minimum_eigensolvers.minimum_eigensolver import MinimumEigensolver, MinimumEigensolverResult
...
class VQE(VariationalAlgorithm, MinimumEigensolver):
    ...
    def __init__(
        ...
        estimator_next: BaseEstimator | None = None,
        *,
        ...
        Args:
            ...
            optimizer: A classical optimizer to find the minimum energy. This can either be a
                ...
                protocol.
            estimator_next: The estimator used to calculate the next point in the minimization
                process. Used to avoid making a new call to the hardware backend. Defaults to None.
        ...
        self.estimator_next = estimator_next
        self.ansatz = ansatz
    ...
    def compute_minimum_eigenvalue(
        ...
        if self.estimator_next:
            evaluate_energy_next = self._get_evaluate_energy(self.ansatz, operator, for_next = True)
        ...
        if callable(self.optimizer):
            ...
            if self.estimator_next:
                optimizer_result = self.optimizer.minimize(
                    fun=evaluate_energy, x0=initial_point, fun_next = evaluate_energy_next, jac=evaluate_gradient, bounds=bounds
                )
            else:
                optimizer_result = self.optimizer.minimize(
                    fun=evaluate_energy, x0=initial_point, jac=evaluate_gradient, bounds=bounds
    ...
    def _get_evaluate_energy(
        ...
        def evaluate_energy(parameters: np.ndarray) -> np.ndarray | float:
            ...
            try:
                if for_next:
                    job = self.estimator_next.run(batch_size * [ansatz], batch_size * [operator], parameters)
                else:
